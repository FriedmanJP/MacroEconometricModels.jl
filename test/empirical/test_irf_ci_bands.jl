...
# ── VAR bootstrap CI ────────────────────────────────────────────────────
_tprint("=" ^ 72)
_tprint("  IRF Confidence Interval Band Width Diagnostic")
_tprint("=" ^ 72)
...
for h in [1, 5, 10, 15, 20, 30, 40]
    ...
    _tprint(@sprintf("  h=%-4d  %10.4f  %10.4f  %10.4f", h, w...))
end
...
_tprint(@sprintf("  %-6s  %10s  %10s", "h", "Width(1,1)", "Width(2,1)"))
shrink_count = let sc = 0
    for h_idx in 1:(H+1)
        ...
        if h in [0, 1, 2, 5, 10, 15, 20, 25, 30, 35, 40]
            _tprint(@sprintf("  h=%-4d  %10.4f  %10.4f%s", h, w1, w2, flag))
        end
    end
    sc
end
...
_tprint("  Summary")
_tprint("=" ^ 72)
_tprint("  LP CI width shrinkage occurrences (Var1→Var1): $shrink_count / $H")
if shrink_count == 0
    _tprint("  PASS: LP CI bands do not shrink pathologically.")
else
    _tprint("  NOTE: Some shrinkage detected. Minor, non-systematic shrinkage")
    _tprint("        can occur due to changing effective sample size.")
end
_tprint("=" ^ 72)
