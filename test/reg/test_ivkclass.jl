...
# ---------------------------------------------------------------------------------------------
# Fixed over-identified DGP (must reproduce the CSV fed to linearmodels bit-for-bit).
...
# MersenneTwister is NOT stable across Julia versions, so the exact (y,w,xend,z1,z2,z3) that
# linearmodels saw is committed as data/reg_ev36.csv (generated by test/gen_ev_fixtures.jl).
# ---------------------------------------------------------------------------------------------
function _ev36_dgp()
    d = readdlm(joinpath(@__DIR__, "data", "reg_ev36.csv"), ',', Float64)
    n = size(d, 1)
    y = d[:, 1]; w = d[:, 2]; xend = d[:, 3]
    z1 = d[:, 4]; z2 = d[:, 5]; z3 = d[:, 6]
