...
@testset "BB SV-SVAR (#825)" begin
    # Full-T MCEM recovery runs only outside FAST (ubuntu-full gate). Default
    # maxiter=500 × gibbs_draws=100 on T=2000/3000 cost ~4–9 min under macOS
    # coverage and timed out the empirical cell (#828). FAST keeps a capped
    # smoke so Codecov still sees the hetero / show / registry paths.
    ...
    @testset "partial hetero smoke (capped)" begin
        Yp, _, _, _ = generate_sv_var(; n=2, Tobs=FAST ? 400 : 800, rng=Xoshiro(30),
            sigmas=[0.25, 0.0])
        rp = MEM.identify_sv_svar(Yp, 1; hetero=[true, false],
            maxiter=FAST ? 8 : 20, rng=Xoshiro(31))
        @test rp.hetero == BitVector([true, false])
        @test isnan(rp.sigmas[2]) && isnan(rp.rhos[2]) && isnan(rp.mus[2])
        @test all(isnan, rp.H_smooth[:, 2])
        @test all(isfinite, rp.H_smooth[:, 1])
    ...
    @testset "determinism" begin
        Yd, _, _, _ = generate_sv_var(; n=2, Tobs=FAST ? 400 : 1000, rng=Xoshiro(50))
        d1 = MEM.identify_sv_svar(Yd, 1; maxiter=FAST ? 8 : 20, rng=Xoshiro(51))
        d2 = MEM.identify_sv_svar(Yd, 1; maxiter=FAST ? 8 : 20, rng=Xoshiro(51))
        @test d1.B == d2.B
    end

    @testset "registry and compute_Q (#826)" begin
        Y, _, _, _ = generate_sv_var(; n=2, Tobs=FAST ? 400 : 1000, rng=Xoshiro(70))
        m = estimate_var(Y, 1)
