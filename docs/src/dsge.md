...
The replication scripts live in `examples/dynare_replication/`. Each script is self-contained and prints PASS/FAIL for each comparison:

```julia
include("examples/dynare_replication/tier1_rbc_baseline.jl")
include("examples/dynare_replication/tier5_smets_wouters_2007.jl")
```

Reference `.mat` files in `examples/dynare_replication/dynare_results/` are generated by `run_dynare_reference.m` using Octave + Dynare 6.5+. Regenerate after updating the DSGE\_mod source:

```bash
cd examples/dynare_replication && octave --no-gui run_dynare_reference.m
```

### Estimation Replication

The estimation pipeline is validated separately in `examples/dynare_replication/estimation_sw07.jl`, which generates synthetic data from the Smets & Wouters (2007) model at posterior mode and runs Bayesian estimation via SMC. A toy-model suite tests all three samplers (SMC, RWMH, SMC``^2``), Bayes factors, and posterior IRF/FEVD/simulation.
