...
[`KrusellSmithSolution`](@ref), and a bootstrap [`ImpulseResponse`](@ref) via
the two-argument `reproduce(ir, model)` (the source `VARModel` is not retained
on the IRF result). Returns a [`ReproReport`](@ref); `matched` is `missing`
when reproduction cannot be attempted.
"""
reproduce(x) = ReproReport(missing, ReproFieldDiff[], nothing, 0, Threads.nthreads(),
    ...
    "BayesianDSGE, KrusellSmithSolution, and bootstrap ImpulseResponse (reproduce(ir, model)).")
