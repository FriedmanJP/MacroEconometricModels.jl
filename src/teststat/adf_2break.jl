...
The regression is the additive-outlier form — breaks enter the ADF equation as
dummies and the minimum `t`-statistic on `γ` over all admissible break pairs is
taken — so the Narayan-Popp tables, computed for their unobserved-components
statistic, do not apply and previously left the test rejecting a driftless
random walk about 70% of the time at a 5% nominal level (issue #577). Critical
values are now simulated from the null distribution of this statistic
(`ADF_2BREAK_SIM_CV`, regenerated by `test/oracle/gen_lm_adf2break_cvs.jl`) and
interpolated in 1/T over T = 100, 150, 250, 500. They are conditional on the
deterministic search design: `trim = 0.10` and no lag augmentation.
...
- Narayan, P. K., & Popp, S. (2010). A new unit root test with two structural
  ...
  37(9), 1425-1438.
- Lumsdaine, R. L., & Papell, D. H. (1997). Multiple trend breaks and the
...
function adf_2break_test(y::AbstractVector{T};
    ...
    cv_row = _adf_2break_cv_row(model, n)
