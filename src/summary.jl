...
report(x::AndersonRubinCI) = show(stdout, x)
report(x::MontielOleaPfluegerF) = show(stdout, x)
report(x::LPIVARBand) = show(stdout, x)
report(x::VARForecast) = show(stdout, x)
report(x::BVARForecast) = show(stdout, x)
report(x::TVPVARPosterior) = show(stdout, x)
report(x::MFVARPosterior) = show(stdout, x)
report(x::GLPHyperparameters) = show(stdout, x)
report(x::ConditionalForecast) = show(stdout, x)
...
report(x::ModelSpec) = show(stdout, x)
...
report(m::PoissonModel) = show(stdout, m)            # EV-19 (#427)
report(io::IO, m::PoissonModel) = show(io, m)        # EV-19 (#427)
report(m::NegBinModel) = show(stdout, m)             # EV-19 (#427)
report(io::IO, m::NegBinModel) = show(io, m)         # EV-19 (#427)
report(r::DispersionTest) = show(stdout, r)          # EV-19 (#427)
report(io::IO, r::DispersionTest) = show(io, r)      # EV-19 (#427)
...
report(x::RASResult) = show(stdout, x)
report(x::ExtractionResult) = show(stdout, x)

report(x::PriceModelResult) = show(stdout, x)
report(x::ImpactResult) = show(stdout, x)
report(x::NetworkStatsResult) = show(stdout, x)
...
report(x::BFMisallocation) = show(stdout, x)
