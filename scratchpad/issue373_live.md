...
- [ ] #260 — Disable table cropping in non-TTY output so significance/CI columns are never silently dropped
- [ ] #261 — Suppress empty column-label rows; print legends/notes as wrapped text instead of tables
- [ ] #262 — Rewrite _fmt as an @sprintf fixed-decimal formatter (aligned columns, -0.0 normalization, scientific fallback)
- [ ] #263 — _coef_table dust/reference-row guard (print — and no stars) plus degenerate-fit warning banner
- [ ] #264 — Deduplicate _select_horizons endpoint so display no longer emits doubled horizon rows
- [ ] #265 — Establish shared display conventions (label dictionary, one _fmt_pvalue, CV order, Yes/No, AIC-scale labels, IRF-horizon note)
- [ ] #266 — Add missing report() dispatches and wrap bare NamedTuple/Vector returns in display types
- [ ] #267 — Content standard: every report() must end with an estimates table (LP family, DSGE solutions, nowcast headline, factor forecast, Uhlig/Arias)
- [ ] #268 — Unify on one display dialect: port DSGESpec/CT/OLG/panel-summary to the shared table infra and add io::IO plumbing package-wide
- [ ] #269 — Fix CI/quantile/SE label mismatches (BVAR 3%/97%, ARIMA SE column, volatility CI note, LP-FEVD units, HonestDiD dead column, VECM title)
- [ ] #270 — Fix johansen_test cointegration-rank off-by-one (rank = r+1 on rejection) and dedupe the two rank selectors
- [ ] #271 — Investigate panel between-R² printing exactly 1.0 on unrelated datasets
- [ ] #272 — Flag suspicious GLP hyperparameters in nowcast BVAR report (lambda ≈ 148 suggests unconverged optimizer)
- [ ] #273 — Display one-off fixes batch (HD shock labels, |Loading| column, ARIMA order grid, normality names, BN note, HA log10 label, news scaling, KS progress)
- [ ] #274 — Make LaTeX backend output compilable-quality (booktabs rules, math-mode/escaped headers, skip legend tables); fix HTML header-split artifacts
- [ ] #275 — Add display regression tests and a goldens suite (no `omitted`, no -0.0, no raw e-notation, stars present)
